...
void WidgetTreeNode::Paint() {
	...
	if (!parent_->is_root_) {
		...
		if (!parent_->widget_->is_showing()) {
			assert(parent_->widget_->visible_sc_ == widget_->visible_sc_);
		}
	}
...
void WidgetTreeNode::OnModifyVisible(bool b) {
	widget_->resume_visible_ = b;
	widget_->visible_sc_.set_control(b);
...
void WidgetTreeNode::OnModifyChildrenVisible(bool b) {
	widget_->visible_sc_.set_control(b ? widget_->resume_visible_ : b);
	for (auto& child : children_) {
