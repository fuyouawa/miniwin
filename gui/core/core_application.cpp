...
namespace fugui {
int CoreApplication::Execute(bool hide_main_window, std::u8string_view window_title)
{
    is_executing_ = true;
    OnInitizlize();
...
// You may manually call LoadIniSettingsFromMemory() to load settings from your own storage.
...
#endif
    {
        ...
        glfwGetWindowContentScale(window, &cur_scale_.x, &cur_scale_.y);
        if (cur_scale_ != prev_scale_) {
...
void CoreApplication::DoFps() {
    ...
    if (sleep_time > delta_time_) {
        Sleep(static_cast<DWORD>(sleep_time - delta_time_));
    }
}
}