...
void WidgetEngine::CloseWidget(Widget* widget) {
	assert(IsInUiThread());
	auto node = widget->p_->owning_node_;
	assert(node);
	assert(!node->closed_);
...
Widget* WidgetEngine::GetParent(const Widget* const widget) {
	if (auto parent_node = widget->p_->owning_node_->parent_) {
...
void WidgetEngine::ModifyParent(Widget* widget, Widget* new_parent) {
	...
	auto node = widget->p_->owning_node_;
	auto parent = new_parent ? new_parent->p_->owning_node_ : root_;
