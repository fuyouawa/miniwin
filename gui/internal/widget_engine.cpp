...
// 	//TODO Ã¶¾Ù×ª×Ö·û´®
// 	str += std::format("{}{}\n", (int)root->widget_->widget_type(), status);
// 	printf(str.c_str());
...
// 	assert(IsInUiThread());
// 	assert(!widget->impl_->owning_node_);
...
// 	assert(IsInUiThread());
// 	auto node = widget->impl_->owning_node_;
// 	assert(node);
// 	assert(!node->closed_);
...
// 	if (auto parent_node = widget->impl_->owning_node_->parent_) {
...
// 	auto node = widget->impl_->owning_node_;
// 	auto parent = new_parent ? new_parent->impl_->owning_node_ : root_;
//
// 	assert(!node->closed_);
// 	if (node->initialized_) {
...
//
// 	if (*node->widget_->p_->visible_sc_ == b)
// 		return;
//
// 	bool can_set = true;
// 	if (b) {
...
// 		}
// 	}
// 	node->widget_->p_->resume_visible_ = b;
// 	if (can_set) {
