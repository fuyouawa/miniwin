...
void WidgetTreeNode::Paint() {
	...
	if (!parent_->is_root_) {
		...
		if (!parent_->widget_->is_showing()) {
			assert(parent_->widget_->p_->visible_sc_ == widget_->p_->visible_sc_);
		}
	}
...
void WidgetTreeNode::OnModifyVisible(bool b) {
	widget_->p_->resume_visible_ = b;
	widget_->p_->visible_sc_.set_control(b);
...
{
	widget_->p_->visible_sc_.set_control(b ? widget_->p_->resume_visible_ : b);
	for (auto& child : children_) {
